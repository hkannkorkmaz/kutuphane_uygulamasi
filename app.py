...
def listele_excel():
    """Excel’i başlık dahil döndürür; fonksiyon içinde [1:] ile başlık atlanır."""
    wb = openpyxl.load_workbook(dosya_adi)
    sheet = wb.active
    kitaplar = list(sheet.iter_rows(values_only=True))
    # None'ları boş stringe çevir
    kitaplar = [tuple("" if v is None else v for v in satir) for satir in kitaplar]
    return kitaplar

def tree_guncelle(tree_widget):
    """Treeview’i komple yeniden doldur (başlığı atlar)."""
    if not tree_widget:
        return
    ...
    for i, kitap in enumerate(kitaplar[1:], start=1):
        # Beklenen şema: Ad, Yazar, Sayfa, ISBN, Kategori, Durum
        ...
        tree_widget.insert("", "end", values=(i, ad, yazar, sayfa, isbn, kategori, durum))
...
def kitap_bilgisi_getir(isbn, kaynak="OpenLibrary"):
    ...
    if kaynak == "OpenLibrary":
        try:
            ...
            yazar = ", ".join([y.get("name","") for y in yazar_list if isinstance(y, dict)]) or "Bilinmiyor"
...
def kitap_guncelle_gui(tree_widget, kaynak="OpenLibrary"):
    try:
        ...
        wb.save(dosya_adi)
        tree_guncelle(tree_widget)
        messagebox.showinfo("Güncelleme","Tüm ISBN'ler internetten kontrol edildi ve güncellendi.")
    except PermissionError:
        messagebox.showerror("Hata", "Excel dosyası açık. Lütfen kapatıp tekrar deneyin.")

def kitap_duzenle_gui(tree_widget):
    secim = simpledialog.askinteger("Düzenle","Düzenlemek istediğiniz kitabın sıra numarası:")
    kitaplar = listele_excel()
    if not secim or secim < 1 or secim > len(kitaplar[1:]):
        return
    secilen = kitaplar[secim]  # başlık atlandığı için index = secim
    ...
    kategori = simpledialog.askstring("Kategori","Kategori:", initialvalue=secilen[4] if len(secilen)>4 else "")
    durum = simpledialog.askstring("Durum","Durum (Okundu/Okunmadı):", initialvalue=secilen[5] if len(secilen)>5 else "Okunmadı")
    if not ad or not yazar or not isbn:
        return
    wb = openpyxl.load_workbook(dosya_adi)
    sheet = wb.active
    for col, val in enumerate([ad, yazar, sayfa, isbn, kategori, (durum if durum in ["Okundu","Okunmadı"] else "Okunmadı")], start=1):
...
def kitap_arama_gui(tree_widget):
    ...
    for i, satir in enumerate(kitaplar[1:], start=1):
        ...
        if s in ad or s in yazar or sorgu in isbn:
            durum = satir[5] if len(satir) > 5 else "Okunmadı"
            tree_widget.insert("", "end", values=(i, satir[0], satir[1], satir[2], satir[3], satir[4], durum))
...
def barkod_goster_gui(tree_widget):
    secim = simpledialog.askinteger("Barkod", "Barkod oluşturmak için sıra numarası girin (veya iptal):")
    if not secim:
        return
    kitaplar = listele_excel()
...
def barkod_ile_cek_gui(tree_widget):
    ...
    secim = messagebox.askquestion("Seçim", "Kitabı Manuel ISBN ile eklemek istiyor musunuz?\n(Hayır: Kamera ile)")
    if secim == "yes":
    ...
    # Kamera ile okuma
    kamera_secim = messagebox.askquestion("Kamera Seçimi","Hangi kamerayı kullanmak istersiniz?\nEvet: Dahili Kamera\nHayır: Telefon Kamerası (IP)")
    if kamera_secim == "yes":
        ...
        combobox = ttk.Combobox(ip_dialog, textvariable=url_var, values=gecmis_ip_urller, width=40)
    ...
    isbn = None
    messagebox.showinfo("Bilgi", "Kameraya barkodu gösterin, ESC ile iptal edebilirsiniz.")
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        ...
        for barcode in barcodes:
            ...
            cv2.rectangle(frame,
                          ...
                          (0, 255, 0), 2)
            cv2.putText(frame, isbn, (barcode.rect.left, barcode.rect.top - 10),
...
def goster_kitaplar():
    ...
    kitaplar = listele_excel()
    if len(kitaplar) <= 1:
        tk.Label(center_frame, text="Kütüphane boş!", font=("Arial", 14, "bold"),
                 fg=THEME["fg_text"], bg=THEME["bg_main"]).pack(expand=True)
        return None
    ...
    if view_mode.get() == "tree":
        ...
        # doldur
        for i, kitap in enumerate(kitaplar[1:], start=1):
            ad = kitap[0]; yazar = kitap[1]; sayfa = kitap[2]; isbn = kitap[3]
            kategori = kitap[4] if len(kitap) > 4 else ""
            durum = kitap[5] if len(kitap) > 5 else "Okunmadı"
            tree_widget.insert("", "end", values=(i, ad, yazar, sayfa, isbn, kategori, durum))
        ...
        for idx, kitap in enumerate(kitaplar[1:]):  # başlığı atla
            ...
            frm.grid(row=idx // cols, column=idx % cols, padx=15, pady=15, sticky="nsew")

            kapak = kapak_getir(isbn)
            lbl_text = f"{ad}\n{yazar}\n{sayfa} sayfa\nISBN: {isbn}\nKategori: {kategori}"
            lbl = tk.Label(frm, text=lbl_text, justify="center", bg=card_bg, fg=text_fg, font=epic_font)
            ...
            durum_lbl = tk.Label(frm, textvariable=durum_var, bg=card_bg, fg="#ffd166", font=("Garamond", 11, "bold"))
            ...
            tk.Button(
                frm, text="Durumu Değiştir (Okundu/Okunmadı)",
                command=lambda i=isbn: toggle_local(i),
                bg=hover_bg, fg=text_fg, font=("Garamond", 10, "bold"),
                activebackground="#4a4a6a", activeforeground=text_fg
            ).pack(side="top", pady=(0,6))

            # Hover efekti
            def on_enter(e, f=frm): f.config(bg=hover_bg)
            def on_leave(e, f=frm): f.config(bg=card_bg)
...
for b, cmd in btns:
    tk.Button(
        left_frame, text=b, width=22, command=cmd,
        bg=THEME["bg_card"], fg=THEME["fg_text"], font=THEME["btn_font"],
        activebackground=THEME["bg_hover"], activeforeground=THEME["fg_text"]
    ).pack(pady=6)

# Çıkış butonu
tk.Button(
    left_frame, text="Çıkış", width=22, command=root.destroy,
    bg="#b33a3a", fg="white", font=("Garamond", 11, "bold"),
    activebackground="#992f2f", activeforeground="white"
).pack(pady=14)
